...
from boltz.data.msa.colabfold_local import (
    compute_msa_colabfold_local,
    compute_msa_colabfold_local_batched,
)
from boltz.data.msa.mmseqs2 import run_mmseqs2
from boltz.data.msa.mmseqs_local import _compute_nucleotide_msa
...
def _collect_msa_sequences(
    path: Path,
    ccd: dict,
    mol_dir: Path,
    boltz2: bool,
) -> tuple[str, dict[str, str]]:
    """Parse input file and return sequences needing MSA generation.

    Parameters
    ----------
    path : Path
        Path to input file (.fasta or .yaml).
    ccd : dict
        The CCD dictionary.
    mol_dir : Path
        The molecule directory.
    boltz2 : bool
        Whether to use Boltz2 parsing.

    Returns
    -------
    tuple[str, dict[str, str], dict[str, str]]
        (target_id, {msa_id: sequence} for proteins, {msa_id: sequence} for nucleotides).
    ...
    try:
        ...
        for chain in target.record.chains:
            ...
            if chain.mol_type == prot_id:
                protein_seqs[msa_id] = target.sequences[entity_id]
            elif chain.mol_type in (dna_id, rna_id):
                nucleotide_seqs[msa_id] = target.sequences[entity_id]
        return (target_id, protein_seqs, nucleotide_seqs)
...
def process_input(  # noqa: C901, PLR0912, PLR0915, D103
    ...
    structure_dir: Path,
    records_dir: Path,
    use_colabfold_search: bool = False,
    mmseqs_binary: Optional[str] = None,
    mmseqs_db_dir: Optional[str] = None,
    mmseqs_gpu_device: Optional[int] = None,
    mmseqs_threads: Optional[int] = None,
    mmseqs_sensitivity: float = 7.5,
    mmseqs_temp_dir: Optional[str] = None,
    ...
    try:
        ...
        for chain in target.record.chains:
            ...
            if chain.mol_type == prot_id:
                ...
                nucleotide_to_generate[msa_id] = target.sequences[entity_id]
                chain.msa_id = msa_dir / f"{msa_id}.csv"
            else:
                # Ligands and other non-polymer chains don't get MSA
                chain.msa_id = -1

        # Generate protein MSA
        if to_generate and not use_msa_server and not use_colabfold_search:
            msg = "Missing MSA's in input and neither --use_msa_server nor --use_colabfold_search flag set."
            raise RuntimeError(msg)  # noqa: TRY301

        # Nucleotide MSA requires local mmseqs (server doesn't support it)
        if nucleotide_to_generate and not use_colabfold_search:
            click.echo(
                "DNA/RNA MSA generation requires --use_colabfold_search. "
                "Nucleotide chains will proceed without MSA."
            )
            for chain in target.record.chains:
                if chain.mol_type in (dna_id, rna_id) and chain.msa_id != -1:
                    msa_id_str = str(chain.msa_id)
                    if msa_id_str.endswith(".csv") and not Path(msa_id_str).exists():
                        chain.msa_id = -1
            nucleotide_to_generate = {}

        all_to_generate = {**to_generate, **nucleotide_to_generate}
        if all_to_generate:
            # Check if MSAs were already generated by batch pre-pass
            already_done = all(
                (msa_dir / f"{msa_id}.csv").exists() for msa_id in all_to_generate
            )
            if already_done:
                click.echo(f"MSAs for {path.name} already generated by batch search, skipping.")
            elif use_colabfold_search:
                n_prot = len(to_generate)
                n_nuc = len(nucleotide_to_generate)
                msg = f"Generating MSA for {path} with {n_prot} protein + {n_nuc} nucleotide entities."
                click.echo(msg)
                if to_generate:
                    compute_msa_colabfold_local(
                        data=to_generate,
                        ...
                        mmseqs_db_dir=mmseqs_db_dir or "",
                        gpu_enabled=True,
                        gpu_device=mmseqs_gpu_device,
                        threads=mmseqs_threads,
                        temp_dir=mmseqs_temp_dir,
                        sensitivity=mmseqs_sensitivity,
                    )
                if nucleotide_to_generate:
                    _compute_nucleotide_msa(
                        nucleotide_data=nucleotide_to_generate,
                        target_id=target_id,
                        msa_dir=msa_dir,
                        mmseqs_binary=mmseqs_binary or "mmseqs",
                        mmseqs_db_dir=mmseqs_db_dir or "",
                        rna_db_dir=rna_db_dir,
                        threads=mmseqs_threads or 16,
                        temp_dir=mmseqs_temp_dir,
                        sensitivity=mmseqs_sensitivity,
                    )
            else:
...
def process_inputs(
    ...
    # ---- Phase 1: Batched MSA pre-generation for --use_colabfold_search ----
    # Collect all protein and nucleotide sequences across all inputs and run
    # ONE batched GPU search (protein, ColabFold pipeline) + CPU search
    # (nucleotide, nt_rna/rfam/rnacentral).
    if use_colabfold_search and data:
        click.echo("Collecting sequences for batched local MSA search...")
        all_targets: dict[str, dict[str, str]] = {}
        all_nuc_targets: dict[str, dict[str, str]] = {}
        for path in data:
            target_id, prot_seqs, nuc_seqs = _collect_msa_sequences(
                path, ccd, mol_dir, boltz2
            )
            if prot_seqs:
                all_targets[target_id] = prot_seqs
            if nuc_seqs:
                all_nuc_targets[target_id] = nuc_seqs
        ...
        if all_targets:
            compute_msa_colabfold_local_batched(
                ...
                temp_dir=mmseqs_temp_dir,
                sensitivity=mmseqs_sensitivity,
                batch_size=mmseqs_batch_size,
            )

        if all_nuc_targets:
            click.echo("Generating nucleotide MSAs (CPU)...")
            for target_id, nuc_seqs in all_nuc_targets.items():
                _compute_nucleotide_msa(
                    nucleotide_data=nuc_seqs,
                    target_id=target_id,
                    msa_dir=msa_dir,
                    mmseqs_binary=mmseqs_binary or "mmseqs",
                    mmseqs_db_dir=mmseqs_db_dir or "",
    ...
    # ---- Phase 2: Per-file processing (MSA will be skipped if already on disk) ----
    ...
    process_input_partial = partial(
        ...
        mmseqs_gpu_device=mmseqs_gpu_device,
...
@click.option(
    ...
    "mmseqs path in parallel. Requires a GPU-capable mmseqs binary.",
)
@click.option(
    "--mmseqs_binary",
    type=str,
...
@click.option(
    "--mmseqs_db_dir",
    type=str,
    help="Directory containing the ColabFold MMseqs databases for local GPU search.",
    default=None,
)
@click.option(
    "--mmseqs_gpu_device",
    type=int,
    help="Specific GPU device for MMseqs2 search (sets CUDA_VISIBLE_DEVICES). Default: all GPUs.",
    default=None,
)
@click.option(
    "--mmseqs_threads",
    type=int,
    help="CPU threads for MMseqs2 non-GPU operations. Default: all cores.",
    default=None,
)
@click.option(
    "--mmseqs_sensitivity",
    type=float,
    help="MMseqs2 search sensitivity (1-7.5). Higher finds more remote homologs. Default: 7.5.",
    default=7.5,
)
@click.option(
    "--mmseqs_temp_dir",
    type=str,
    help="Fast local storage for MMseqs2 temp files (e.g., /scratch on HPC). Default: system temp.",
    default=None,
)
@click.option(
    "--mmseqs_batch_size",
    type=int,
    help="Max sequences per GPU search batch for --use_colabfold_search. Default: 512.",
    default=512,
)
@click.option(
    "--rna_db_dir",
    type=str,
    help="Directory containing nucleotide MMseqs2 databases (nt_rna, rfam, rnacentral) "
    "for DNA/RNA MSA generation. Auto-detected as sibling 'mmseqs_rna/' of --mmseqs_db_dir if not set.",
    default=None,
)
@click.option(
    "--use_potentials",
...
def predict(  # noqa: C901, PLR0915, PLR0912
    ...
    warnings.filterwarnings(
        "ignore", ".*pin_memory.*is deprecated.*"
    )
    warnings.filterwarnings(
        "ignore", ".*is_pinned.*is deprecated.*"
    )
    ...
    # Validate mutually exclusive MSA modes
    if use_msa_server and use_colabfold_search:
        msg = (
            "Cannot use both --use_msa_server and --use_colabfold_search. "
            "Please choose one MSA generation method."
        )
    ...
    # Configure local ColabFold search if enabled
    if use_colabfold_search:
        ...
        if mmseqs_db_dir is None:
            mmseqs_db_dir = os.environ.get("BOLTZ_MMSEQS_DB_DIR")
        if mmseqs_db_dir is None:
            msg = (
                "ColabFold MMseqs database directory not specified. Use "
                "--mmseqs_db_dir or set BOLTZ_MMSEQS_DB_DIR. See the README "
                "for database setup instructions."
            )
            raise click.UsageError(msg)

        available_dbs = detect_databases(mmseqs_db_dir)
        click.echo(f"Local ColabFold search enabled: binary={mmseqs_binary}")
        click.echo(f"MMseqs2 databases: {mmseqs_db_dir}")
        click.echo(f"Available databases: {', '.join(available_dbs.keys())}")
        if mmseqs_gpu_device is not None:
            click.echo(f"MMseqs2 GPU device: {mmseqs_gpu_device}")
        if mmseqs_temp_dir is not None:
            click.echo(f"MMseqs2 temp dir: {mmseqs_temp_dir}")
    ...
    process_inputs(
        ...
        mmseqs_sensitivity=mmseqs_sensitivity,
        mmseqs_temp_dir=mmseqs_temp_dir,
        mmseqs_batch_size=mmseqs_batch_size,
        rna_db_dir=rna_db_dir,
    )
    t_msa_end = time.time()
    msa_time = t_msa_end - t_msa_start
    click.echo(f"\n[Benchmark] MSA generation + preprocessing: {msa_time:.1f}s")
    ...
    if filtered_manifest.records:
        ...
        click.echo(f"\n[Benchmark] Inference: {inference_time:.1f}s")
        click.echo(f"[Benchmark] MSA generation + preprocessing: {msa_time:.1f}s")
        click.echo(f"[Benchmark] Inference: {inference_time:.1f}s")
        click.echo(f"[Benchmark] Total: {msa_time + inference_time:.1f}s")
        click.echo(f"[Benchmark] Num inputs: {len(filtered_manifest.records)}")
        click.echo(f"[Benchmark] Avg per input: {(msa_time + inference_time) / len(filtered_manifest.records):.2f}s")
