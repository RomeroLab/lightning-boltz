...
from boltz.data.msa.mmseqs_local import compute_msa_local, compute_msa_local_batched
...
def process_input(  # noqa: C901, PLR0912, PLR0915, D103
    ...
    try:
        ...
        if to_generate and not use_msa_server and not use_mmseqs_gpu:
            msg = "Missing MSA's in input and neither --use_msa_server nor --use_mmseqs_gpu flag set."
            raise RuntimeError(msg)  # noqa: TRY301

        # Nucleotide MSA requires local mmseqs (server doesn't support it)
        if nucleotide_to_generate and not use_mmseqs_gpu:
            click.echo(
                "DNA/RNA MSA generation requires --use_mmseqs_gpu. "
        ...
        if all_to_generate:
            ...
            if already_done:
                ...
                compute_msa_local(
                    ...
                    nucleotide_data=nucleotide_to_generate or None,
...
def process_inputs(
    ...
    # ---- Phase 1: Batched MSA pre-generation for --use_mmseqs_gpu ----
    # Collect all protein and nucleotide sequences across all inputs and run
    # ONE batched GPU search (protein) + CPU search (nucleotide).
    if use_mmseqs_gpu and data:
        click.echo("Collecting sequences for batched GPU MSA search...")
        ...
        if all_targets or all_nuc_targets:
            compute_msa_local_batched(
                targets=all_targets,
                msa_dir=msa_dir,
                msa_pairing_strategy=msa_pairing_strategy,
                mmseqs_binary=mmseqs_binary,
                ...
                nucleotide_targets=all_nuc_targets or None,
                rna_db_dir=rna_db_dir,
            )
...
@click.option(
    "--use_mmseqs_gpu",
    is_flag=True,
    help="Use local MMseqs2-GPU for MSA generation instead of the ColabFold server. "
    "Requires mmseqs binary with GPU support and local padded databases. "
    "See docs/MMSEQS_GPU.md for setup instructions.",
)
@click.option(
    "--mmseqs_binary",
    type=str,
...
@click.option(
    "--mmseqs_db_dir",
    type=str,
    help="Directory containing MMseqs2 padded databases for local GPU search.",
    default=None,
)
@click.option(
    "--mmseqs_gpu_device",
...
@click.option(
    "--mmseqs_batch_size",
    type=int,
    help="Max sequences per GPU search batch for --use_mmseqs_gpu. Default: 512.",
    default=512,
)
@click.option(
    "--rna_db_dir",
...
def predict(  # noqa: C901, PLR0915, PLR0912
    ...
    if use_msa_server and use_mmseqs_gpu:
        msg = (
    ...
    # Configure local MMseqs2-GPU if enabled
    if use_mmseqs_gpu:
        ...
        if mmseqs_db_dir is None:
            msg = (
                "MMseqs2 database directory not specified. Use --mmseqs_db_dir "
                "or set BOLTZ_MMSEQS_DB_DIR environment variable. "
                "See docs/MMSEQS_GPU.md for database setup instructions."
            )
            raise click.UsageError(msg)

        available_dbs = detect_databases(mmseqs_db_dir)
        click.echo(f"MMseqs2-GPU enabled: binary={mmseqs_binary}")
