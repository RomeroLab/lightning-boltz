...
def _collect_msa_sequences(
    ...
    -------
    tuple[str, dict[str, str]]
        (target_id, {msa_id: sequence}) for proteins needing MSA.
...
def process_input(  # noqa: C901, PLR0912, PLR0915, D103
    ...
    try:
        ...
        if to_generate:
            ...
            already_done = all(
                (msa_dir / f"{msa_id}.csv").exists() for msa_id in to_generate
            )
            if already_done:
...
def process_inputs(
    ...
    # Collect all protein sequences across all inputs and run ONE batched
    # GPU search instead of per-file searches.
    if use_mmseqs_gpu and data:
        click.echo("Collecting sequences for batched GPU MSA search...")
        all_targets: dict[str, dict[str, str]] = {}
        for path in data:
            target_id, seqs = _collect_msa_sequences(path, ccd, mol_dir, boltz2)
            if seqs:
                all_targets[target_id] = seqs
